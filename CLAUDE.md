...
4. `ARCHITECTURE_FOUNDATION.md` — approved order of the 19 architecture items.
5. `releases/alpha-007/MECHANISMS_77.json` — all 77 required mechanisms.
6. `compact_state/CORE.md`, `SAVE.json`, `SYNC.json` — current compact computer
   state and synchronization status.
7. `runtime/README.md` and `runtime/docs/` — external local runtime contract.

## Authority rules

- Build: `0.7.2-r3-architecture-foundation`, artifact revision `3`.
- The architecture foundation contains 19 ordered items: 17 required rules and
...
`CLAUDE.md` first and to report the build ID, 19 architecture items, mechanism
...
`canon/CORE_MASTER.json`, `releases/alpha-007/MECHANISMS_77.json`, and the three
files under `compact_state/`.
